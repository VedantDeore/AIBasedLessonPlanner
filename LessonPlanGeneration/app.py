...
# Topic Modeling: Use Latent Dirichlet Allocation (LDA) or other clustering techniques to group the text into topics.
# Text Classification: Train a model (e.g., a neural network or support vector machine) to classify each segment of text into one of the syllabus topics.
...
#             # Get the lecture number to delete
#             lecture_number = int(request.form.get('lecture_number'))
            
#             # Remove the row based on the lecture number (adjusting for zero-based index)
#             if 1 <= lecture_number <= len(lesson_plan):
#                 lesson_plan.pop(lecture_number - 1)  # Delete the row
            
#             # Recalculate lecture numbers after deletion
#             for i, entry in enumerate(lesson_plan):
#                 entry['lecture_number'] = i + 1
...
#          # Renumber lecture numbers for all rows
#         for i, entry in enumerate(lesson_plan):
#             entry['lecture_number'] = i + 1
...
@app.route('/final_plan', methods=['GET','POST'])
def final_plan():
    # Path to the lesson plan JSON file
    
    
    # # Check if lesson plan JSON exists
    # if os.path.exists(lesson_plan_file):
    #     # Load the existing lesson plan
    #     with open(lesson_plan_file, 'r') as f:
    #         lesson_plan = json.load(f)
    if True:
        # Generate a new lesson plan from the syllabus and calendar
        with open('data/calendar_data.json', 'r') as f:
            calendar_data = json.load(f)
        with open('data/syllabus_data.json', 'r') as f:
            syllabus_data = json.load(f)
        
        # Extract data and generate valid lecture dates
        start_date = calendar_data['events']['start_of_semester']
        end_date = calendar_data['events']['end_of_semester']
        lecture_days = calendar_data['lecture_days']
        lecture_times = calendar_data['lecture_times']
        holidays = calendar_data['holidays']
        events = calendar_data['events']
        syllabus_text = syllabus_data.get('syllabus_text', "")
        ...
        day_time_mapping = {day: time for day, time in zip(lecture_days, lecture_times)} 


        # Add additional fields and populate the lesson plan
        method_default = "PPT/White Board"
        student_activity_default = "Discussion, Solving of Problem"
        assessment_tools = ["Quiz", "HA", "Tutorial", "ESE"]
        time_slot = "10:00 AM - 12:00 PM"
        day_mapping = {0: 'Monday', 1: 'Tuesday', 2: 'Wednesday', 3: 'Thursday', 4: 'Friday', 5: 'Saturday', 6: 'Sunday'}
        section_flag = {"I": False, "II": False}

        for i, entry in enumerate(lesson_plan):
            date_obj = datetime.strptime(entry['date'], "%Y-%m-%d")
            day_name = day_mapping[date_obj.weekday()]
            entry['day'] = day_mapping[date_obj.weekday()]
            entry['lecture_number'] = i + 1
            # entry['time_slot'] = time_slot
            entry['time_slot'] = day_time_mapping.get(day_name, "9:00 AM - 10:00 AM")  # Default if day not found
            entry['method'] = method_default
            entry['student_activity'] = student_activity_default
            entry['assessment_tool'] = random.choice(assessment_tools)
            entry['remarks'] = ""
            if "SECTION-I" in entry['topic'] and not section_flag["I"]:
                entry['section'] = "SECTION-I"
                section_flag["I"] = True
        ...
        lesson_plan_file = 'data/lesson_plan.json'
        # Save the initial lesson plan
        with open(lesson_plan_file, 'w') as f:
            json.dump(lesson_plan, f, indent=4)
            
    lesson_plan_file = 'data/lesson_plan.json'

    # Handle POST requests to update the lesson plan
    if request.method == 'POST':
        action = request.form.get('action')
        if action == 'add_row':
            # Handle row addition logic
            lecture_number = int(request.form.get('lecture_number'))
            new_row = {
                'date': request.form.get(f'date_{lecture_number + 1}', ''),
                'time_slot': request.form.get(f'time_slot_{lecture_number + 1}', ''),
            ...
            lesson_plan.insert(lecture_number, new_row)
        elif action == 'delete_row':
            lecture_number = int(request.form.get('lecture_number'))
            if 1 <= lecture_number <= len(lesson_plan):
                lesson_plan.pop(lecture_number - 1)
        elif action == 'add_column':
            column_name = request.form.get('column_name')
        ...
        # Recalculate lecture numbers and save the updated plan
        for i, entry in enumerate(lesson_plan):
            entry['lecture_number'] = i + 1
        with open(lesson_plan_file, 'w') as f:
            json.dump(lesson_plan, f, indent=4)
